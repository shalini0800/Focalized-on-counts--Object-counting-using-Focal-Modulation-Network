...
class PatchEmbed_Rev(nn.Module):
    ...
    def __init__(self, img_size=(96, 96), patch_size=4, in_chans=96, embed_dim=96, use_conv_embed=False, norm_layer=None, is_stem=False):
        super().__init__()
        ...
        if use_conv_embed:
            ...
            self.proj = nn.ConvTranspose2d(embed_dim, in_chans, kernel_size=kernel_size, stride=stride, padding=padding)
        else:
...
class FocalNet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.patch_embed = PatchEmbed_Rev(
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=embed_dim[i_layer], 
                               ...
                               norm_layer=norm_layer, 
                               downsample=PatchEmbed_Rev if (i_layer < self.num_layers - 1) else None,
...
@register_model
def focalnet_base_iso_decoder(pretrained=False, **kwargs):
    model = FocalNet(depths=[8], patch_size=16, embed_dim=768, focal_levels=[3], focal_windows=[3], use_layerscale=True, use_postln=True, **kwargs)
    ...
    # checkpoint = torch.load('./data/out/focalnet_base_iso_16.pth', map_location='cpu')
