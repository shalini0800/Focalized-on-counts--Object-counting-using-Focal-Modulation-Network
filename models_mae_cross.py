...
from models.Focal_vit16_Fine_tuning import focalnet_base_iso
...
class SupervisedMAE(nn.Module):
    ...
    def forward_encoder(self, x):
        # print("forward encoder input",x.shape)
        # embed patches
        #x = self.patch_embed(x)
        #print("forward encoder after patch embed",x.shape)

        # add pos embed w/o cls token
        #x = x + self.pos_embed
        #print("forward encoder after pos embed",x.shape)

        # apply Transformer blocks
        for blk in self.blocks:
            x = blk(x)
        # print("forward output before norm",x.shape)
        x = self.norm(x)
        # print("forward encoder output after norm",x.shape)

        return x

    def forward_decoder(self, x, y_, shot_num=3):
        # print("forward decoder input", x.shape)
        # embed tokens
        x = self.decoder_embed(x)
        # print("forward decoder after patch embed",x.shape)
        # add pos embed
        desired_shape = self.decoder_pos_embed.shape[1]
        x = x[:, :desired_shape, ...]
        p=self.decoder_pos_embed
        # print("problem",p.shape)
        x = x + p
        # print("forward decoder after pos embed",x.shape)
